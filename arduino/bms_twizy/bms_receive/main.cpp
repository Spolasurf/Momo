...
void reconnect()
{
    if (millis() - lastTryMqtt > 3000 && WiFi.status() == WL_CONNECTED) {
        SPI.endTransaction();
        if (client.connect("arduinoClient5", "Sandy", "Sandy2018")) {
        }
        else {
...
void loop()
{
   ...
   //     setupCan();
   // }
   if (millis() - lastMillisTest > 1000) {
       Serial.println("test");
       lastMillisTest = millis();
    }
...
void decodeMsg(can_frame msg)
{
    switch (msg.can_id) {
    case 0x059B:
        int pedal = msg.data[3];
        int gang = msg.data[0];

        if (millis() - lastPublishedPedal > 1000) {
            if(publishMsg("sevcon/pedal", pedal)) lastPublishedPedal = millis();
        }
        if (millis() - lastPublishedGang > 1000) {
            if(publishMsg("sevcon/gang", gang)) lastPublishedGang = millis();   
        }
        break;
    }
}
